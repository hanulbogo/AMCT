function H = rgbhist_fast(I,Foreground,nBins,Nind)
...
%  I=double(applycform(uint8(I),cform));
...
label = reshape(Foreground,[size(I,1)*size(I,2) 1]);
...
for i=1:size(I,1)*size(I,2)
    if label(i) >0
        p=double(im(i,:));
        p=floor(p/(256/nBins))+1;
        H(p(1),p(2),p(3))=H(p(1),p(2),p(3))+label(i);
    end
end
