function TrackingMain_NRD_Hops()
global SupFlag;
global InputPath;
global OutputPath;
global OFroot;
...
%For each Category
for cc =[2,length(Cats)-1]%[3,4,5,6,8,9,11]%1:length(Cats)%[1:8,11]%1:length(Cats)%[1:6 8:9]%10:length(Cats)]
    ...
    [Salpix,Sal,bbox]=InitialSegmentation_Hops(Params);
    ...
    %     save([salcpath num2str(ff,'%04d') '_pp.mat'],'PP');
    sff=ff;
    ...
    for ff =sff+1: nFrame
        ...
        [BasicSal, PP]=PropAMC_OneDirection_Hops(img,PP,OF_PC,OF_CP,SupFlag);
        
        bbox = PP.bbox;
        save([salcpath num2str(ff,'%04d') '.mat'], 'BasicSal','bbox');
        %         imwrite(PP.RGB,[outimgcpath num2str(ff,'%04d') '.tiff']);
